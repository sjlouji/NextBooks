...
const { accountValidation, updateAccount } = require('../../middleware/validators/admin/accountValidation')
...
module.exports = {
    ...
    createAccount: async function(req,res){
        const { account_name, account_type, initialBalance, account_provider, account_id, user } = req.body

        // Validating Payload with JOI
        const {error} = accountValidation(req.body);
        if(error) return res.status(400).json({"error":error.details[0].message});

        const validateUser = await User.findOne({_id: user})
        if(!validateUser) return res.status(404).json({"error":"User Does not Exist"})

        //  Finding If a Account Number Exists
        const validateAccountId = await Account.findOne({user: validateUser._id,account_id: account_id})
    ...
    updateAccount: async function(req,res){
        let { id, account_name, account_type, account_provider, user } = req.body
        ...
        //  Validating if Accound exists             
        const validate = await Account.findOne({_id: id, user: validateUser._id})
        if(!validate) return res.status(404).json({"error":"Account does Not Exists"})
